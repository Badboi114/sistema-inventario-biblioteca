...
def importar_libros_y_tesis():
    ...
    with transaction.atomic():
        for idx, row in df_libros.iterrows():
            ...
            # Evitar duplicados por código o título+autor+año
            clave = (raw_codigo, titulo.strip().lower())
            if raw_codigo in codigos_libros or clave in titulos_libros:
                continue
            codigos_libros.add(raw_codigo)
            titulos_libros.add(clave)
            datos = {
    ...
    with transaction.atomic():
        for idx, row in df_tesis.iterrows():
            ...
            clave = (raw_codigo, titulo.strip().lower())
            if raw_codigo in codigos_tesis or clave in titulos_tesis:
                continue
            codigos_tesis.add(raw_codigo)
            titulos_tesis.add(clave)
